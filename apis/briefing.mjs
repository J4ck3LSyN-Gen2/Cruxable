...
// === Tier 6: Cyber Intelligence ===
import { briefing as nist } from './sources/nist.mjs';
import { briefing as cisa } from './sources/cisa.mjs';
...
export async function fullBriefing() {
  ...
  const results = await Promise.allSettled([
    ...
    // Tier 6: Cyber Intelligence
    runSource('NIST', nist, process.env.NIST_API_KEY),
    runSource('CISA', cisa),
    runSource('EPSS', epss),
    runSource('GreyNoise', greynoise, process.env.GREYNOISE_API_KEY),
  ]);
